  ...
  public class PierComponent : MonoBehaviour
  {
    ...
    public void Awake()
    {
      ...
      if (ZNetView.m_forceDisableInit)
      {
        ...
        if (Object.op_Implicit((Object) component1) && Object.op_Implicit((Object) component1.m_new))
          component1.m_new.SetActive(false);
        this.m_segmentObject.SetActive(false);
        foreach (Collider componentsInChild in this.m_segmentObject.GetComponentsInChildren<Collider>(true))
    ...
    {
      ((Component) this).transform.position = new Vector3(((Component) this).transform.position.x, ZoneSystem.instance.m_waterLevel + this.m_baseOffset, ((Component) this).transform.position.z);
      RaycastHit[] raycastHitArray = Physics.RaycastAll(((Component) this).transform.position, Vector3.down, 100f, this.m_terrainLayer);
      float num1 = 0.0f;
      for (int index = 0; index < raycastHitArray.Length; ++index)
      {
        if ((double) num1 < (double) ((RaycastHit) ref raycastHitArray[index]).distance)
          num1 = ((RaycastHit) ref raycastHitArray[index]).distance;
      }
      int num2 = Mathf.CeilToInt(num1 / this.m_segmentHeight);
      for (int count = this.m_segmentObjects.Count; count < num2; ++count)
        this.m_segmentObjects.Add(Object.Instantiate<GameObject>(this.m_segmentObject, ((Component) this).transform.position, Quaternion.identity, ((Component) this).transform));
      ...
      for (int index = 0; index < this.m_segmentObjects.Count; ++index)
      {
        ...
        segmentObject.transform.localPosition = new Vector3(0.0f, -this.m_segmentHeight * (float) (index + 1), 0.0f);
