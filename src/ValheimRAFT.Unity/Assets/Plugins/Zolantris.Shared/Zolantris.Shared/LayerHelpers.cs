...
// ReSharper disable NamespaceStyle
  namespace Zolantris.Shared
  {
    public static class LayerHelpers
    {
      ...
      [UsedImplicitly]
      public static List<int> GetActiveLayers(LayerMask mask, string layerVariant = "activeLayers")
      {
        var activeLayers = new List<int>();
        var activeLayersNames = new List<string>();
        ...
        for (var i = 0; i < 32; i++)
        {
          ...
          activeLayersNames.Add(name);
          activeLayers.Add(i); // Add the layer index to the list
        }

#if DEBUG
        var layersString = string.Join(", ", activeLayersNames);
        LoggerProvider.LogDebugDebounced($"{layerVariant}: {layersString}");
#endif
