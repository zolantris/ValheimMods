...
namespace Eldritch.Core
{
  ...
  #region hunt creeping logic
    ...
    public void MoveAlongDirectionWhileFacing(Vector3 moveDir, Vector3 facePoint, float speed, float lerpRate, float turnSpeed)
    {
      // 1) Face the target
      var toFace = facePoint - transform.position;
      toFace.y = 0f;
      if (toFace.sqrMagnitude > 0.0001f)
        RotateTowardsDirection(toFace, turnSpeed);

      // 2) Move along the retreat/path direction WITHOUT rotating to it
      moveDir.y = 0f;
      if (moveDir.sqrMagnitude < 0.0001f) return;
      ...
      var newVel = Vector3.Lerp(currentVel, targetVel, Mathf.Clamp01(lerpRate * Time.fixedDeltaTime));
      var delta = newVel - currentVel;
      delta.y = 0f;
