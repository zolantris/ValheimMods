...
public class BoardingRampComponent : MonoBehaviour, Interactable, Hoverable
{
  ...
  private List<Transform> m_ropeAttach1 = new();

  private List<Transform> m_ropeAttach2 = new();
  ...
  private void Awake()
  {
    ...
    {
      m_rope1.gameObject.SetActive(false);
      m_rope2.gameObject.SetActive(false);
  ...
  {
    if (m_nview.IsOwner()) SetState((BoardingRampState)state);
  }
  ...
  {
    if ((bool)m_nview && m_nview.IsOwner()) SetSegmentCount(segmentCount);
  }
  ...
  private void CreateSegments()
  {
    ...
    {
      var go = Instantiate(m_segmentObject,
        transform.position + new Vector3(0f, (0f - m_segmentHeight) * (float)i,
  ...
  private void CheckRampFloor()
  {
    ...
    else
    {
      if (m_state != BoardingRampState.Opening &&
          m_state != BoardingRampState.Open) return;
  ...
  private void UpdateRamp()
  {
    ...
    else
    {
      ...
      m_desiredRotation =
        Quaternion.Euler(
          (Quaternion.Inverse(transform.rotation) * m_desiredRotation)
          .eulerAngles.x, 0f, 0f);
      m_rotSpeed =
        Mathf.Clamp(
    ...
    if (m_lastBridgeProgress != bridgeProgress)
    {
      ...
      for (var i = 1; i < m_segmentObjects.Count; i++)
      {
        var segmentProgress =
  ...
  public bool Interact(Humanoid user, bool hold, bool alt)
  {
    if (alt)
    {
      if (m_editPanel == null) m_editPanel = new EditRampComponent();
