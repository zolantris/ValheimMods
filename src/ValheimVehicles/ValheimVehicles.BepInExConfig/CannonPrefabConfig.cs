...
public class CannonPrefabConfig : BepInExBaseConfig<CannonPrefabConfig>
{
  ...
  public override void OnBindConfig(ConfigFile config)
  {
    ...
    Cannonball_ShieldGeneratorDamageMultiplier = config.BindUnique(VehicleCannonsSection, "ShieldGeneratorDamageMultiplier", 0.05f, ConfigHelpers.CreateConfigDescription("Set the damage cannons do to shield generators. Shield generators should soak alot more damage. So consider using a low number for cannonballs otherwise only ~3 hits can collapse a generator", true, false, new AcceptableValueRange<float>(0, 1)));
