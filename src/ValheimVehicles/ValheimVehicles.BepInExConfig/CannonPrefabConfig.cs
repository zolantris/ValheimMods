...
public class CannonPrefabConfig : BepInExBaseConfig<CannonPrefabConfig>
{
  ...
  public static ConfigEntry<bool> DEBUG_CannonballUnlimitedAmmo = null!;
  ...
  public static ConfigEntry<float> CannonballSolidDamage = null!;
  public static ConfigEntry<float> CannonballExplosiveDamage = null!;
  ...
  public void SetupCannonball(ConfigFile config)
  {
    ...
    CannonBallInventoryWeight = config.BindUnique(CannonballsSection, "CannonBallInventoryWeight", 4f, ConfigHelpers.CreateConfigDescription("Set the weight of cannonballs. For realism 12-48lbs for these cannons.", true, false, new AcceptableValueRange<float>(0, 100)));
    Cannonball_ExplosionAudioVolume = config.BindUnique(CannonballsSection, "Cannonball_ExplosionAudioVolume", 1f, ConfigHelpers.CreateConfigDescription("Allows customizing cannon reload audio volume", false, false));
    Cannonball_ExplosiveRadius = config.BindUnique(CannonballsSection, "Cannonball_ExplosionRadius", 7.5f, ConfigHelpers.CreateConfigDescription("Allows customizing cannonball explosion radius/aoe. Large sizes will lag out objects like rocks", false, false, new AcceptableValueRange<float>(3f, 20f)));
    Cannonball_SolidBaseDamage = config.BindUnique(CannonballsSection, "Cannonball_SolidShell_BaseDamage", 85f, ConfigHelpers.CreateConfigDescription("Allows customizing cannonball solid hit damage", false, false, new AcceptableValueRange<float>(25f, 500f)));
    Cannonball_ExplosiveBaseDamage = config.BindUnique(CannonballsSection, "Cannonball_ExplosiveShell_BaseDamage", 50f, ConfigHelpers.CreateConfigDescription("Allows customizing cannonball explosion hit AOE damage. The damage is uniform across the entire radius.", false, false, new AcceptableValueRange<float>(25f, 500f)));
    DEBUG_CannonballUnlimitedAmmo = config.BindUnique(CannonballsSection, "DEBUG_CannonballUnlimitedAmmo", false, ConfigHelpers.CreateConfigDescription("Allows unlimited ammo for cannons.", true, false));
    HasCannonballWindAudio = config.BindUnique(CannonballsSection, "Cannonball_HasWindAudio", true, ConfigHelpers.CreateConfigDescription("Allows enable cannonball wind audio - which can be heard if a cannonball passes nearby.", false, false));
    CannonballWindAudioVolume = config.BindUnique(CannonballsSection, "Cannonball_WindAudioVolume", 0.2f, ConfigHelpers.CreateConfigDescription("Allows customizing cannonball wind audio - which can be heard if a cannonball passes nearby. Recommended below 0.2f", false, false));
    CannonballSolidDamage = config.BindUnique(CannonballsSection, "Cannonball_SolidDamage", 30f, ConfigHelpers.CreateConfigDescription("Set the amount of damage a solid cannon ball does. This value is multiplied by the velocity of the cannonball around 90 at max speed decreasing to 20 m/s at lowest hit damage level.", false, false));
    CannonballExplosiveDamage = config.BindUnique(CannonballsSection, "Cannonball_ExplosiveDamage", 30f, ConfigHelpers.CreateConfigDescription("Set the amount of damage a explosive cannon ball does. This damage includes both the AOE and hit. AOE will do same damage on top of the impact of the shot.", false, false));
