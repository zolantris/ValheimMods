...
public class CultivatableComponent : MonoBehaviour
{
  ...
  {
    int myid = ZdoPersistManager.Instance.GetOrCreatePersistentID(m_nview.m_zdo);
    ...
    {
      ZDO zdo = ZdoPersistManager.Instance.GetZDO(list[i]);
      if (zdo == null)
      {
        continue;
      }
  ...
  {
    AddNewChild(ZdoPersistManager.Instance.GetOrCreatePersistentID(m_nview.m_zdo), child);
  }
  ...
  public static void AddChild(int parent, ZNetView child)
  {
    ...
    AddChild(parent, ZdoPersistManager.Instance.GetOrCreatePersistentID(child.m_zdo));
  }
