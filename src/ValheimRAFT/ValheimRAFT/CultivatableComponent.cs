...
public class CultivatableComponent : MonoBehaviour
{
	...
	private static readonly int MBCultivatableParentIdHash = "MBCultivatableParentId".GetStableHashCode();
	...
	{
		Vector2 uvoffset = (base.transform.parent ? new Vector2(0f - base.transform.localPosition.x, 0f - base.transform.localPosition.z) : new Vector2(0f - base.transform.position.x, 0f - base.transform.position.z));
		uvoffset /= textureScale;
		float uvrotation = (base.transform.parent ? (0f - base.transform.localEulerAngles.y) : (0f - base.transform.eulerAngles.y));
		uvrotation /= 360f;
	...
	private void OnDestroyed()
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			if ((bool)obj)
			{
				...
				{
					ZLog.LogWarning($" gameObject == obj || transform.IsChildOf(obj.transform) {base.gameObject} == {obj} || {base.transform.IsChildOf(obj.transform)}");
					continue;
				}
	...
	public static int GetParentID(ZNetView netview)
	{
		...
		if (id == 0)
		{
			...
			if (zdoid != ZDOID.None)
			{
				...
				id = ((zdoparent == null) ? ZDOPersistantID.ZDOIDToId(zdoid) : ZDOPersistantID.Instance.GetOrCreatePersistantID(zdoparent));
