...
public class BoardingRampComponent : MonoBehaviour, Interactable, Hoverable
{
	...
	private void CreateSegments()
	{
		...
		{
			GameObject go = Object.Instantiate(m_segmentObject, base.transform.position + new Vector3(0f, (0f - m_segmentHeight) * (float)i, m_segmentLength * (float)i), Quaternion.identity, m_ramp.transform);
			go.transform.localRotation = Quaternion.identity;
			go.transform.localScale = new Vector3(1f + (float)i * 0.0001f, 1f + (float)i * 0.0001f, 1f + (float)i * 0.0001f);
	...
	{
		int hitCount = Physics.RaycastNonAlloc(start, dir, m_rayHits, dist, m_groundRayMask, QueryTriggerInteraction.Ignore);
		int hitIndex = 0;
		bool found = false;
		for (int i = 0; i < hitCount; i++)
		{
			if (!m_rayHits[i].transform.IsChildOf(base.transform.parent ?? base.transform) && (!found || m_rayHits[hitIndex].distance > m_rayHits[i].distance))
			{
				hitIndex = i;
				found = true;
			}
	...
	private void CheckRampFloor()
	{
		...
		else
		{
			...
			Vector3 p = m_ramp.transform.TransformPoint(new Vector3(0f, (0f - m_segmentHeight) * (float)m_segments, lineLen));
			...
			else if (!m_lastHitRamp && RaycastNonSelf(p + new Vector3(0f, 5f, 0f), Vector3.down, 1000f, out hitInfo))
			{
	...
	private void UpdateRamp()
	{
		...
		{
			m_desiredRotation = Quaternion.LookRotation((m_hitPosition - (base.transform.position - new Vector3(0f, m_hitDistance * m_segmentHeight * (float)m_segments, 0f))).normalized);
			m_desiredRotation = Quaternion.Euler((Quaternion.Inverse(base.transform.rotation) * m_desiredRotation).eulerAngles.x, 0f, 0f);
		...
		m_winch1.transform.localRotation = Quaternion.Euler(m_stateProgress * 1000f * (float)m_segments, 0f, -90f);
		m_winch2.transform.localRotation = Quaternion.Euler(m_stateProgress * 1000f * (float)m_segments, 0f, -90f);
		...
		if (m_lastBridgeProgress != bridgeProgress)
		{
			...
			{
				float segmentProgress = Mathf.Clamp01(bridgeProgress * (float)m_segmentObjects.Count / (float)i);
				m_segmentObjects[i].transform.position = m_ramp.transform.TransformPoint(new Vector3(0f, (0f - m_segmentHeight) * (float)i, segmentProgress * (m_segmentLength - m_segmentOverlap) * (float)i));
			}
		}
		UpdateRopes();
	}
	...
	{
		string stateChangeDesc = ((m_state == BoardingRampState.Open || m_state == BoardingRampState.Opening) ? "$mb_boarding_ramp_retract" : "$mb_boarding_ramp_extend");
		return Localization.instance.Localize("[<color=yellow><b>$KEY_Use</b></color>] " + stateChangeDesc) + Localization.instance.Localize("\n[<color=yellow><b>$KEY_AltPlace + $KEY_Use</b></color>] $mb_boarding_ramp_edit");
	}
