...
public class PlanBuild_Patch
{
  ...
  {
    if (__instance.GetComponentInParent<VehiclePieceController>())
    {
      __result = true;
      return false;
    }
  ...
  {
    var baseVehicle = originatingPiece.GetComponentInParent<VehiclePieceController>();
    ...
    bool captureCurrentSnapPoints = false, bool keepMarkers = false)
  {
    ...
    foreach (Piece item4 in orderedEnumerable)
    {
      ...
      var vehicleBase = item4.m_nview.GetComponentInParent<VehiclePieceController>();
      if (shipBase)
      {
        Logger.LogDebug(
