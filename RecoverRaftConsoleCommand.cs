...
namespace ValheimRAFT
{
  internal class RecoverRaftConsoleCommand : ConsoleCommand
  {
    ...
    {
      Collider[] colliderArray = Physics.OverlapSphere(((Component) GameCamera.instance).transform.position, 1000f);
      Dictionary<ZDOID, List<ZNetView>> dictionary = new Dictionary<ZDOID, List<ZNetView>>();
      ZLog.Log((object) string.Format("Searching {0}", (object) ((Component) GameCamera.instance).transform.position));
      foreach (Component component1 in colliderArray)
      {
        ZNetView component2 = component1.GetComponent<ZNetView>();
        if (Object.op_Inequality((Object) component2, (Object) null) && component2.m_zdo != null && !Object.op_Implicit((Object) ((Component) component2).GetComponentInParent<MoveableBaseRootComponent>()))
        {
      ...
      ZLog.Log((object) string.Format("Found {0} potential ships to recover.", (object) dictionary.Count));
      if (args.Length != 0 && args[0] == "confirm")
      {
        foreach (ZDOID key in dictionary.Keys)
        {
          List<ZNetView> znetViewList = dictionary[key];
          MoveableBaseShipComponent component = Object.Instantiate<GameObject>(ZNetScene.instance.GetPrefab("MBRaft"), ((Component) znetViewList[0]).transform.position, ((Component) znetViewList[0]).transform.rotation).GetComponent<MoveableBaseShipComponent>();
          foreach (ZNetView netview in znetViewList)
          {
            ...
            ((Component) netview).transform.localRotation = netview.m_zdo.GetQuaternion(MoveableBaseRootComponent.MBRotationHash, Quaternion.identity);
