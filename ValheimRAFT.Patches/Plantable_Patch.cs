...
public class Plantable_Patch
{
	...
	private static IEnumerable<CodeInstruction> UpdatePlacementGhost(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
	{
		...
		{
			if (!list[i].StoresField(AccessTools.Field(typeof(Player), "m_placementStatus")) || !list[i - 1].LoadsConstant(Player.PlacementStatus.NeedCultivated))
			{
				continue;
			}
			...
			list.InsertRange(i - 2, new CodeInstruction[3]
			{
				...
				new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(Plantable_Patch), "IsCultivated")),
	...
	private static IEnumerable<CodeInstruction> Plant_Grow(IEnumerable<CodeInstruction> instructions)
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			if (list[i].Calls(AccessTools.Method(typeof(GameObject), "GetComponent", new Type[0], new Type[1] { typeof(ZNetView) })))
			{
				list[i] = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(Plantable_Patch), "PlantGrowth"));
