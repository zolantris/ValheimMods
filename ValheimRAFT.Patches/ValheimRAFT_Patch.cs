...
public class ValheimRAFT_Patch
{
	...
	[HarmonyPrefix]
	private static bool ShipControlls_RPC_RequestRespons(ShipControlls __instance, long sender, bool granted)
	{
	...
	{
		if ((bool)__instance.m_nview && __instance.m_nview.m_zdo != null && __instance.name.StartsWith("MBRaft"))
		{
	...
	private static bool Ship_UpdateUpsideDmg(Ship __instance)
	{
		...
		{
			__instance.m_body.rotation = Quaternion.Euler(new Vector3(__instance.m_body.rotation.eulerAngles.x, __instance.m_body.rotation.eulerAngles.y, 0f));
		}
		return !mb;
	}
	...
	private static void Ship_UpdateSailSize(Ship __instance)
	{
		...
		for (int i = 0; i < mb.m_baseRoot.m_rudderPieces.Count; i++)
		{
			...
			{
				rudder.m_wheel.localRotation = Quaternion.Slerp(rudder.m_wheel.localRotation, Quaternion.Euler(__instance.m_rudderRotationMax * (0f - __instance.m_rudderValue) * rudder.m_wheelRotationFactor, 0f, 0f), 0.5f);
			}
		}
	}
	...
	private static bool Ship_FixedUpdate(Ship __instance)
	{
		...
		mb.m_flags = (MoveableBaseShipComponent.MBFlags)__instance.m_nview.m_zdo.GetInt("MBFlags", (int)mb.m_flags);
		...
		if (__instance.m_players.Count == 0 || mb.m_flags.HasFlag(MoveableBaseShipComponent.MBFlags.IsAnchored))
		{
		...
		float averageWaterHeight = (waterLevel + waterLevel2 + waterLevel3 + waterLevel4 + waterLevel5) / 5f;
		...
		if (!(currentDepth > __instance.m_disableLevel))
		{
			...
			__instance.m_body.AddForceAtPosition(vector9 * num3, worldCenterOfMass, ForceMode.VelocityChange);
			...
			__instance.m_body.angularVelocity -= __instance.m_body.angularVelocity * __instance.m_angularDamping * num4;
			float num7 = 0.15f;
			float num8 = 0.5f;
			...
			__instance.m_body.AddForceAtPosition(Vector3.up * f2 * num3, vector2, ForceMode.VelocityChange);
			__instance.m_body.AddForceAtPosition(Vector3.up * f3 * num3, vector3, ForceMode.VelocityChange);
			__instance.m_body.AddForceAtPosition(Vector3.up * f4 * num3, vector4, ForceMode.VelocityChange);
			...
			if (mb.m_targetHeight > 0f)
			{
				...
				float centerforce = GetUpwardsForce(mb.m_targetHeight, centerpos.y + __instance.m_body.velocity.y, mb.m_liftForce);
				__instance.m_body.AddForceAtPosition(Vector3.up * centerforce, centerpos, ForceMode.VelocityChange);
			}
		}
		else if (mb.m_targetHeight > 0f)
		{
			mb.UpdateStats(flight: true);
			Vector3 side1 = __instance.m_floatCollider.transform.position + __instance.m_floatCollider.transform.forward * __instance.m_floatCollider.size.z / 2f;
			Vector3 side2 = __instance.m_floatCollider.transform.position - __instance.m_floatCollider.transform.forward * __instance.m_floatCollider.size.z / 2f;
			Vector3 side3 = __instance.m_floatCollider.transform.position - __instance.m_floatCollider.transform.right * __instance.m_floatCollider.size.x / 2f;
			Vector3 side4 = __instance.m_floatCollider.transform.position + __instance.m_floatCollider.transform.right * __instance.m_floatCollider.size.x / 2f;
			...
			float side1force = GetUpwardsForce(mb.m_targetHeight, side1.y + corner1curforce.y, mb.m_balanceForce);
			float side2force = GetUpwardsForce(mb.m_targetHeight, side2.y + corner2curforce.y, mb.m_balanceForce);
			float side3force = GetUpwardsForce(mb.m_targetHeight, side3.y + corner3curforce.y, mb.m_balanceForce);
			float side4force = GetUpwardsForce(mb.m_targetHeight, side4.y + corner4curforce.y, mb.m_balanceForce);
			float centerforce2 = GetUpwardsForce(mb.m_targetHeight, centerpos2.y + __instance.m_body.velocity.y, mb.m_liftForce);
			__instance.m_body.AddForceAtPosition(Vector3.up * side1force, side1, ForceMode.VelocityChange);
			__instance.m_body.AddForceAtPosition(Vector3.up * side2force, side2, ForceMode.VelocityChange);
			__instance.m_body.AddForceAtPosition(Vector3.up * side3force, side3, ForceMode.VelocityChange);
			__instance.m_body.AddForceAtPosition(Vector3.up * side4force, side4, ForceMode.VelocityChange);
			__instance.m_body.AddForceAtPosition(Vector3.up * centerforce2, centerpos2, ForceMode.VelocityChange);
	...
	private static void ApplySailForce(Ship __instance, float num5)
	{
		...
		Vector3 stearoffset = __instance.m_floatCollider.transform.position - __instance.m_floatCollider.transform.forward * __instance.m_floatCollider.size.z / 2f;
		float num7 = num5 * __instance.m_stearVelForceFactor;
		__instance.m_body.AddForceAtPosition(__instance.transform.right * num7 * (0f - __instance.m_rudderValue) * Time.fixedDeltaTime, stearoffset, ForceMode.VelocityChange);
		Vector3 stearforce = Vector3.zero;
		switch (__instance.m_speed)
		{
		case Ship.Speed.Slow:
			stearforce += __instance.transform.forward * __instance.m_backwardForce * (1f - Mathf.Abs(__instance.m_rudderValue));
			break;
		...
		if (__instance.m_speed == Ship.Speed.Back || __instance.m_speed == Ship.Speed.Slow)
		{
			...
			stearforce += __instance.transform.right * __instance.m_stearForce * (0f - __instance.m_rudderValue) * num6;
		}
		__instance.m_body.AddForceAtPosition(stearforce * Time.fixedDeltaTime, stearoffset, ForceMode.VelocityChange);
	}
	...
	[HarmonyTranspiler]
	private static IEnumerable<CodeInstruction> WearNTear_AttachShip(IEnumerable<CodeInstruction> instructions)
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			{
				list[i] = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ValheimRAFT_Patch), "AttachRigidbodyMoveableBase"));
				break;
			}
		}
		return list;
	...
	private static IEnumerable<CodeInstruction> UpdatePlacementGhost(IEnumerable<CodeInstruction> instructions)
	{
		...
		for (int i = 0; i < list.Count; i++)
		{
			...
			{
				list[i] = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ValheimRAFT_Patch), "RelativeEuler"));
			}
		}
		return list;
	}
	...
	private static Quaternion RelativeEuler(float x, float y, float z)
	{
		...
		MoveableBaseRootComponent mbr = m_lastRayPiece.GetComponentInParent<MoveableBaseRootComponent>();
		if (!mbr)
		{
			return rot;
		}
	...
	private static bool Character_GetStandingOnShip(Character __instance, ref Ship __result)
	{
		...
		if ((bool)__instance.m_lastGroundBody)
		{
			...
			if (!__result)
			{
				MoveableBaseRootComponent mb = __instance.m_lastGroundBody.GetComponentInParent<MoveableBaseRootComponent>();
		...
		for (int i = 0; i < list.Count; i++)
		{
			if (list[i].operand != null && list[i].operand.ToString() == "UnityEngine.GameObject Instantiate[GameObject](UnityEngine.GameObject, UnityEngine.Vector3, UnityEngine.Quaternion)")
			{
				list.InsertRange(i + 2, new CodeInstruction[3]
				{
					...
					new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(ValheimRAFT_Patch), "PlacedPiece"))
				});
				break;
			}
		}
	...
	[HarmonyPatch(typeof(Player), "PieceRayTest")]
	[HarmonyPrefix]
	private static bool PieceRayTest(Player __instance, ref bool __result, ref Vector3 point, ref Vector3 normal, ref Piece piece, ref Heightmap heightmap, ref Collider waterSurface, bool water)
	{
		...
		if ((bool)mbr)
		{
			...
			Quaternion localDir = ((Character)__instance).m_lookYaw * Quaternion.Euler(__instance.m_lookPitch, 0f - mbr.transform.rotation.eulerAngles.y + yawOffset, 0f);
			...
			{
				MoveableBaseRootComponent mbrTarget = hitInfo.collider.GetComponentInParent<MoveableBaseRootComponent>();
				if ((bool)mbrTarget)
				{
					point = hitInfo.point;
	...
	{
		if ((bool)((Character)__instance).m_lastGroundCollider && ((Character)__instance).m_lastGroundTouch < 0.3f)
		{
			MoveableBaseRootComponent.AddDynamicParent(((Character)__instance).m_nview, ((Character)__instance).m_lastGroundCollider.gameObject);
		}
	}

	[HarmonyPatch(typeof(Player), "PieceRayTest")]
	[HarmonyPostfix]
	private static void PieceRayTestPostfix(Player __instance, ref bool __result, ref Vector3 point, ref Vector3 normal, ref Piece piece, ref Heightmap heightmap, ref Collider waterSurface, bool water)
	{
	...
	[HarmonyPrefix]
	private static bool FindHoverObject(Player __instance, ref GameObject hover, ref Character hoverCreature)
	{
		hover = null;
		hoverCreature = null;
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			if (Vector3.Distance(__instance.m_eye.position, raycastHit.point) < __instance.m_maxInteractDistance)
			{
				...
				else if ((bool)raycastHit.collider.attachedRigidbody && !raycastHit.collider.attachedRigidbody.GetComponent<MoveableBaseRootComponent>())
				{
	...
	{
		if (__instance.m_character is Player player && player.IsAttached() && (bool)player.m_attachPoint && (bool)player.m_attachPoint.parent)
		{
	...
	{
		if (__instance.IsAttached() && (bool)__instance.m_attachPoint && (bool)__instance.m_attachPoint.parent)
		{
			RopeLadderComponent ladder = __instance.m_attachPoint.parent.GetComponent<RopeLadderComponent>();
			if ((bool)ladder)
			{
	...
	[HarmonyPrefix]
	private static bool SetControls(Player __instance, Vector3 movedir, bool attack, bool attackHold, bool secondaryAttack, bool block, bool blockHold, bool jump, bool crouch, bool run, bool autoRun)
	{
		if (__instance.IsAttached() && (bool)__instance.m_attachPoint && (bool)__instance.m_attachPoint.parent)
		{
			if (movedir.x == 0f && movedir.y == 0f && !jump && !crouch && !attack && !attackHold && !secondaryAttack && !block)
			{
				RopeLadderComponent ladder = __instance.m_attachPoint.parent.GetComponent<RopeLadderComponent>();
				if ((bool)ladder)
				{
			...
			if ((bool)rudder && __instance.m_doodadController != null)
			{
				...
				if (__instance.m_doodadController is ShipControlls shipControlls && (bool)shipControlls.m_ship)
				{
					MoveableBaseShipComponent mb = shipControlls.m_ship.GetComponent<MoveableBaseShipComponent>();
					if ((bool)mb)
					{
						...
						else if (mb.m_flags.HasFlag(MoveableBaseShipComponent.MBFlags.IsAnchored) && movedir != Vector3.zero)
						{
	...
	{
		if (!__instance.m_nview.IsValid() || !__instance.m_nview.IsOwner() || __instance.m_jumpTimer < 0.1f)
		{
			return false;
		}
