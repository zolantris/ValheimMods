...
public class PlanBuild_Patch
{
	...
	[HarmonyPrefix]
  	public static bool Capture(Blueprint __instance, ref bool __result,  Selection selection, bool captureCurrentSnapPoints = false, bool keepMarkers = false)
	{
		...
		foreach (ZDOID item2 in selection)
		{
			...
			if (gameObject.name.StartsWith("piece_bpcenterpoint"))
			{
				...
				{
					Jotunn.Logger.LogWarning($"Multiple center points! Ignoring @ {gameObject.transform.position}");
				}
				if (!keepMarkers)
				{
			...
			if (gameObject.name.StartsWith("piece_bpterrainmod"))
			{
				...
				TerrainModEntry item = new TerrainModEntry(zDO.GetString("shape"), gameObject.transform.position, float.Parse(zDO.GetString("radius"), CultureInfo.InvariantCulture), int.Parse(zDO.GetString("rotation"), CultureInfo.InvariantCulture), float.Parse(zDO.GetString("smooth"), CultureInfo.InvariantCulture), zDO.GetString("paint"));
				list3.Add(item);
				if (!keepMarkers)
				{
		...
		foreach (Piece item4 in orderedEnumerable)
		{
			...
			if (shipBase)
			{
				ZLog.DevLog($"used ship calc for position {item4.m_nview.m_zdo.GetVec3(MoveableBaseRootComponent.MBPositionHash, Vector3.zero)}");
			...
			{
				text = string.Format("{0}:{1}:{2}", component6.m_nview.m_zdo.GetString("item"), component6.m_nview.m_zdo.GetInt("variant"), component6.m_nview.m_zdo.GetInt("quality"));
			}
			...
			__instance.PieceEntries[num6++] = new PieceEntry(text2, item4.m_category.ToString(), pos, rotation, text, localScale);
		}
		...
		{
			__instance.TerrainMods[num7++] = new TerrainModEntry(item5.shape, item5.GetPosition() - vector, item5.radius, item5.rotation, item5.smooth, item5.paint);
		}
