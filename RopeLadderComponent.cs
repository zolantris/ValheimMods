...
namespace ValheimRAFT
{
  public class RopeLadderComponent : MonoBehaviour, Interactable, Hoverable
  {
    ...
    public string GetHoverText() => Localization.instance.Localize("[<color=yellow><b>$KEY_Use</b></color>] $mb_rope_ladder_use");
    ...
    private void ClimbLadder(Player player)
    {
      ...
      else
      {
        ...
        this.m_attachPoint.localPosition = new Vector3(this.m_attachPoint.localPosition.x, this.ClampOffset(this.m_attachPoint.parent.InverseTransformPoint(((Component) player).transform.position).y), this.m_attachPoint.localPosition.z);
        ((Character) player).AttachStart(this.m_attachPoint, (GameObject) null, true, false, false, "Movement", Vector3.zero, (Transform) null);
      }
    }

    private void UpdateSteps()
    {
      if (!Object.op_Implicit((Object) this.m_stepObject))
        return;
      ...
      ((Vector3) ref point).\u002Ector(((Component) this.m_attachPoint).transform.position.x, 0.0f, ((Component) this.m_attachPoint).transform.position.z);
      Vector3 vector3_1;
      // ISSUE: explicit constructor call
      ((Vector3) ref vector3_1).\u002Ector(((Component) this.m_attachPoint).transform.position.x, ((Component) this).transform.position.y, ((Component) this.m_attachPoint).transform.position.z);
      foreach (RaycastHit raycastHit in Physics.RaycastAll(new Ray(vector3_1, Vector3.op_UnaryNegation(((Component) this.m_attachPoint).transform.up)), this.m_ladderHeight, RopeLadderComponent.rayMask))
      {
        if (!Object.op_Equality((Object) ((RaycastHit) ref raycastHit).collider, (Object) this.m_collider) && !Object.op_Implicit((Object) ((Component) ((RaycastHit) ref raycastHit).collider).GetComponentInParent<Character>()) && (double) ((RaycastHit) ref raycastHit).distance < (double) this.m_ladderHeight)
        {
          this.m_ladderHeight = ((RaycastHit) ref raycastHit).distance;
          point = ((RaycastHit) ref raycastHit).point;
        }
      }
      if (Object.op_Implicit((Object) this.m_mbroot) && Object.op_Implicit((Object) this.m_mbroot.m_moveableBaseShip) && (double) this.m_mbroot.m_moveableBaseShip.m_targetHeight > 0.0 && !this.m_mbroot.m_moveableBaseShip.m_flags.HasFlag((Enum) MoveableBaseShipComponent.MBFlags.IsAnchored) && (double) point.y < (double) this.m_mbroot.GetColliderBottom())
      {
        point.y = this.m_mbroot.GetColliderBottom();
        Vector3 vector3_2 = Vector3.op_Subtraction(point, vector3_1);
        this.m_ladderHeight = ((Vector3) ref vector3_2).magnitude;
      ...
      else if ((double) point.y < (double) ZoneSystem.instance.m_waterLevel)
      {
        ...
        float num = ((Vector3) ref vector3_3).magnitude + 2f;
      ...
      if (this.m_ghostObject)
      {
        ...
        this.m_ghostAttachPoint.SetPosition(1, Vector3.op_Addition(((Component) this.m_attachPoint).transform.position, Vector3.op_Multiply(Vector3.op_UnaryNegation(((Component) this.m_attachPoint).transform.up), this.m_ladderHeight)));
      }
      ...
      while (this.m_steps.Count < num1)
      {
        GameObject gameObject = Object.Instantiate<GameObject>(this.m_stepObject, ((Component) this).transform);
        this.m_steps.Add(gameObject);
        gameObject.transform.localPosition = new Vector3(0.0f, -this.m_stepDistance * (float) this.m_steps.Count, 0.0f);
      }
      this.m_ropeLine.useWorldSpace = false;
      this.m_ropeLine.SetPosition(0, new Vector3(0.4f, 0.0f, 0.0f));
      this.m_ropeLine.SetPosition(1, new Vector3(0.4f, -this.m_stepDistance * (float) this.m_steps.Count, 0.0f));
      this.m_ropeLine.SetPosition(2, new Vector3(-0.4f, -this.m_stepDistance * (float) this.m_steps.Count, 0.0f));
      this.m_ropeLine.SetPosition(3, new Vector3(-0.4f, 0.0f, 0.0f));
      if (this.m_ghostObject)
        return;
      this.m_collider.size = new Vector3(1f, this.m_ladderHeight, 0.1f);
      ((Component) this.m_collider).transform.localPosition = new Vector3(0.0f, (float) (-(double) this.m_ladderHeight / 2.0), 0.0f);
    }
    ...
    public void UpdateIK(Animator animator)
    {
      ...
      if (this.m_targetLeft == RopeLadderComponent.INVALID_STEP && this.m_targetRight == RopeLadderComponent.INVALID_STEP && (double) this.m_currentMoveDir != 0.0)
      {
        if ((double) this.m_currentMoveDir > 0.0 && this.m_currentLeft < this.m_currentRight || (double) this.m_currentMoveDir < 0.0 && this.m_currentLeft > this.m_currentRight || !this.m_lastMovedLeft)
        {
          this.m_targetLeft = num1 + ((double) this.m_currentMoveDir > 0.0 ? this.m_stepOffsetUp : this.m_stepOffsetDown);
          this.m_leftMoveTime = Time.time;
          this.m_lastMovedLeft = true;
        }
        else
        {
          this.m_targetRight = num1 + ((double) this.m_currentMoveDir > 0.0 ? this.m_stepOffsetUp : this.m_stepOffsetDown);
          this.m_rightMoveTime = Time.time;
          this.m_lastMovedLeft = false;
        }
      }
      Vector3 vector3_1 = ((Component) this).transform.TransformPoint(new Vector3(-0.3f, (float) (this.m_currentLeft + 2) * this.m_stepDistance, -0.1f));
      Vector3 vector3_2 = ((Component) this).transform.TransformPoint(new Vector3(-0.2f, (float) this.m_currentLeft * this.m_stepDistance, -0.3f));
      Vector3 vector3_3 = ((Component) this).transform.TransformPoint(new Vector3(0.3f, (float) (this.m_currentRight + 2) * this.m_stepDistance, -0.1f));
      Vector3 vector3_4 = ((Component) this).transform.TransformPoint(new Vector3(0.2f, (float) this.m_currentRight * this.m_stepDistance, -0.3f));
      if (this.m_targetLeft != RopeLadderComponent.INVALID_STEP)
      {
        Vector3 vector3_5 = ((Component) this).transform.TransformPoint(new Vector3(-0.3f, (float) (this.m_targetLeft + 3) * this.m_stepDistance, 0.0f));
        Vector3 vector3_6 = ((Component) this).transform.TransformPoint(new Vector3(-0.2f, (float) this.m_targetLeft * this.m_stepDistance, 0.0f));
        float num2 = Mathf.Clamp01((float) (((double) Time.time - (double) this.m_leftMoveTime) * ((double) this.m_ladderMoveSpeed / (double) this.m_stepDistance)));
      ...
      {
        Vector3 vector3_7 = ((Component) this).transform.TransformPoint(new Vector3(0.3f, (float) (this.m_targetRight + 3) * this.m_stepDistance, 0.0f));
        Vector3 vector3_8 = ((Component) this).transform.TransformPoint(new Vector3(0.2f, (float) this.m_targetRight * this.m_stepDistance, 0.0f));
        float num3 = Mathf.Clamp01((float) (((double) Time.time - (double) this.m_rightMoveTime) * ((double) this.m_ladderMoveSpeed / (double) this.m_stepDistance)));
    ...
    public void MoveOnLadder(Player player, float movedir)
    {
      ...
      if ((double) movedir > 0.0)
        ...
        y -= this.m_ladderMoveSpeed * Time.deltaTime;
      this.m_attachPoint.localPosition = new Vector3(this.m_attachPoint.localPosition.x, this.ClampOffset(y), this.m_attachPoint.localPosition.z);
