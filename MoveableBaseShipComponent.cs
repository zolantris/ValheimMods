...
namespace ValheimRAFT
{
  public class MoveableBaseShipComponent : MonoBehaviour
  {
    ...
    public void Awake()
    {
      ...
      this.m_nview.Register<bool>("SetAnchor", (Action<long, bool>) ((sender, state) => this.RPC_SetAnchor(sender, state)));
      this.m_nview.Register<bool>("SetVisual", (Action<long, bool>) ((sender, state) => this.RPC_SetVisual(sender, state)));
    ...
    public void UpdateVisual()
    {
      if (this.m_nview.m_zdo == null)
        return;
      this.m_flags = (MoveableBaseShipComponent.MBFlags) this.m_nview.m_zdo.GetInt("MBFlags", (int) this.m_flags);
      ((Component) ((Component) this).transform.Find("ship/visual")).gameObject.SetActive(!this.m_flags.HasFlag((Enum) MoveableBaseShipComponent.MBFlags.HideMesh));
      ((Component) ((Component) this).transform.Find("interactive")).gameObject.SetActive(!this.m_flags.HasFlag((Enum) MoveableBaseShipComponent.MBFlags.HideMesh));
    }
    ...
    private void FirstTimeCreation()
    {
      ...
      for (float num1 = -1f; (double) num1 < 1.0099999904632568; num1 += 2f)
      {
        ...
        {
          Vector3 vector3 = ((Component) this).transform.TransformPoint(new Vector3(num1, 0.45f, num2));
          this.m_baseRoot.AddNewPiece(Object.Instantiate<GameObject>(prefab, vector3, ((Component) this).transform.rotation).GetComponent<ZNetView>());
        }
      }
    }
    ...
    {
      this.m_flags = state ? this.m_flags | MoveableBaseShipComponent.MBFlags.IsAnchored : this.m_flags & ~MoveableBaseShipComponent.MBFlags.IsAnchored;
    ...
    {
      this.m_flags = state ? this.m_flags | MoveableBaseShipComponent.MBFlags.HideMesh : this.m_flags & ~MoveableBaseShipComponent.MBFlags.HideMesh;
      this.m_nview.m_zdo.Set("MBFlags", (int) this.m_flags);
      this.UpdateVisual();
    }
