  ...
  public class Teleport_Patch
  {
    ...
    public static void TeleportToActivePosition(TeleportWorld __instance, ZDOID playerId)
    {
      ZDO zdo = ZDOMan.instance.GetZDO(
        __instance.m_nview.m_zdo.GetConnectionZDOID((ZDOExtraData.ConnectionType)1));
      if (zdo == null)
        return;
      ...
      IEnumerable<CodeInstruction> instructions)
    {
      ...
      for (int index = 0; index < list.Count; ++index)
      {
        if (CodeInstructionExtensions.LoadsField(list[index],
          ...
          list[index] = new CodeInstruction(OpCodes.Call,
            (object)AccessTools.Method(typeof(Teleport_Patch), "GetTeleportTargetPos", (Type[])null,
              (Type[])null));
        if (CodeInstructionExtensions.StoresField(list[index],
    ...
    private static void SetIsTeleporting(Player __instance, bool isTeleporting)
    {
      ...
      if (zdo != null)
      {
        ...
        ((Component)__instance).transform.position =
          Teleport_Patch.GetTeleportPosition(((Component)instance).gameObject);
      }
