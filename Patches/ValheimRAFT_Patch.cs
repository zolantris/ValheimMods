...
namespace ValheimRAFT.Patches
{
  ...
  public class ValheimRAFT_Patch
  {
    ...
    private static bool Ship_UpdateUpsideDmg(Ship __instance)
    {
      ...
      if (Object.op_Implicit((Object) component) && (double) ((Component) __instance).transform.up.y < 0.0)
      {
    ...
    private static void Ship_UpdateSailSize(Ship __instance)
    {
      ...
      for (int index = 0; index < component.m_baseRoot.m_mastPieces.Count; ++index)
      {
        ...
        {
          if (Vector3.op_Inequality(mastPiece.m_sailObject.transform.localScale, __instance.m_sailObject.transform.localScale))
    ...
    private static bool Ship_FixedUpdate(Ship __instance)
    {
      ...
      component.m_flags = (MoveableBaseShipComponent.MBFlags) __instance.m_nview.m_zdo.GetInt("MBFlags", (int) component.m_flags);
      ...
      if (__instance.m_players.Count == 0 || component.m_flags.HasFlag((Enum) MoveableBaseShipComponent.MBFlags.IsAnchored))
      {
      ...
      float waterLevel1 = Floating.GetWaterLevel(worldCenterOfMass, ref __instance.m_previousCenter);
      ...
      float num1 = (float) (((double) waterLevel1 + (double) waterLevel2 + (double) waterLevel3 + (double) waterLevel4 + (double) waterLevel5) / 5.0);
      ...
      if ((double) num2 <= (double) __instance.m_disableLevel)
      {
        ...
        ((Vector3) ref vector3_5).\u002Ector(vector3_3.x, waterLevel2, vector3_3.z);
        Vector3 vector3_6;
        // ISSUE: explicit constructor call
        ((Vector3) ref vector3_6).\u002Ector(vector3_4.x, waterLevel3, vector3_4.z);
        Vector3 vector3_7;
        // ISSUE: explicit constructor call
        ((Vector3) ref vector3_7).\u002Ector(vector3_1.x, waterLevel4, vector3_1.z);
        Vector3 vector3_8;
        // ISSUE: explicit constructor call
        ((Vector3) ref vector3_8).\u002Ector(vector3_2.x, waterLevel5, vector3_2.z);
        float num3 = Time.fixedDeltaTime * 50f;
        float num4 = Mathf.Clamp01(Mathf.Abs(num2) / __instance.m_forceDistance);
        Vector3 vector3_9 = Vector3.op_Multiply(Vector3.op_Multiply(Vector3.up, __instance.m_force), num4);
        __instance.m_body.AddForceAtPosition(Vector3.op_Multiply(vector3_9, num3), worldCenterOfMass, (ForceMode) 2);
        ...
        float num7 = velocity1.y * velocity1.y * Mathf.Sign(velocity1.y) * __instance.m_damping * num4;
        ...
        if (magnitude1 > magnitude2)
        {
          ...
          double magnitude3 = (double) ((Vector3) ref velocity2).magnitude;
        ...
        Rigidbody body = __instance.m_body;
        body.angularVelocity = Vector3.op_Subtraction(body.angularVelocity, Vector3.op_Multiply(Vector3.op_Multiply(__instance.m_body.angularVelocity, __instance.m_angularDamping), num4));
        float num10 = 0.15f;
        float num11 = 0.5f;
        ...
        __instance.m_body.AddForceAtPosition(Vector3.op_Multiply(Vector3.op_Multiply(Vector3.up, num16), num3), vector3_1, (ForceMode) 2);
        __instance.m_body.AddForceAtPosition(Vector3.op_Multiply(Vector3.op_Multiply(Vector3.up, num17), num3), vector3_2, (ForceMode) 2);
        __instance.m_body.AddForceAtPosition(Vector3.op_Multiply(Vector3.op_Multiply(Vector3.up, num18), num3), vector3_3, (ForceMode) 2);
        __instance.m_body.AddForceAtPosition(Vector3.op_Multiply(Vector3.op_Multiply(Vector3.up, num19), num3), vector3_4, (ForceMode) 2);
    ...
    [HarmonyPostfix]
    private static void ZDO_Deserialize(ZDO __instance, ZPackage pkg) => ValheimRAFT_Patch.ZDOLoaded(__instance);

    [HarmonyPatch(typeof (ZDO), "Load")]
    [HarmonyPostfix]
    private static void ZDO_Load(ZDO __instance, ZPackage pkg, int version) => ValheimRAFT_Patch.ZDOLoaded(__instance);
    ...
    private static void PlacedPiece(Player player, GameObject gameObject)
    {
      ...
      {
        CultivatableComponent component3 = ((Component) ValheimRAFT_Patch.m_lastRayPiece).GetComponent<CultivatableComponent>();
    ...
    {
      if (!Object.op_Implicit((Object) ((Character) __instance).m_lastGroundCollider) || (double) ((Character) __instance).m_lastGroundTouch >= 0.30000001192092896)
        return;
      MoveableBaseRootComponent.AddDynamicParent(((Character) __instance).m_nview, ((Component) ((Character) __instance).m_lastGroundCollider).gameObject);
    }
      ...
      ref Character hoverCreature)
    {
      ...
      if (Object.op_Implicit((Object) hover) && Object.op_Implicit((Object) RopeAnchorComponent.m_draggingRopeFrom))
      {
          ...
          if (__instance.m_doodadController is ShipControlls doodadController && Object.op_Implicit((Object) doodadController.m_ship))
          {
            ...
            if (Object.op_Implicit((Object) component))
            {
              if (ZInput.GetButton("Jump") || ZInput.GetButton("JoyJump"))
                ...
                component.SetAnchor(!component.m_flags.HasFlag((Enum) MoveableBaseShipComponent.MBFlags.IsAnchored));
              else if (component.m_flags.HasFlag((Enum) MoveableBaseShipComponent.MBFlags.IsAnchored) && Vector3.op_Inequality(movedir, Vector3.zero))
    ...
    {
      if (!__instance.m_nview.IsValid() || !__instance.m_nview.IsOwner() || (double) __instance.m_jumpTimer < 0.10000000149011612)
        return false;
      foreach (ContactPoint contact in collision.contacts)
      {
        ...
        if (!__instance.m_groundContact && (double) vector3_1.y < 0.0 && (double) num < 0.10000000149011612)
        {
          vector3_1 = Vector3.op_Multiply(vector3_1, -1f);
          vector3_2 = ((Component) __instance).transform.position;
        }
        if ((double) vector3_1.y > 0.10000000149011612 && (double) num < (double) __instance.m_collider.radius)
        {
