...
// Type: ValheimRAFT.Patches.Plantable_Patch
...
namespace ValheimRAFT.Patches
{
  ...
  public class Plantable_Patch
  {
      ...
      ILGenerator generator)
    {
      ...
      {
        if (CodeInstructionExtensions.StoresField(list[index1], AccessTools.Field(typeof (Player), "m_placementStatus")) && CodeInstructionExtensions.LoadsConstant(list[index1 - 1], (Enum) (object) (Player.PlacementStatus) 9))
        {
          List<Label> labels = CodeInstructionExtensions.ExtractLabels(list[index1 - 2]);
          object obj = (object) null;
          MethodInfo methodInfo = AccessTools.Method(typeof (Player), "PieceRayTest", (Type[]) null, (Type[]) null);
          for (int index2 = 0; index2 < list.Count; ++index2)
          {
            if (CodeInstructionExtensions.Calls(list[index2], methodInfo) && CodeInstructionExtensions.IsLdloc(list[index2 - 4], (LocalBuilder) null))
            {
          ...
          list.Find((Predicate<CodeInstruction>) (match => match.labels.Contains(sourceJump))).labels.Add(label);
          list.InsertRange(index1 - 2, (IEnumerable<CodeInstruction>) new CodeInstruction[3]
          {
            CodeInstructionExtensions.WithLabels(new CodeInstruction(OpCodes.Ldloc, obj), (IEnumerable<Label>) labels),
    ...
    private static IEnumerable<CodeInstruction> Plant_Grow(IEnumerable<CodeInstruction> instructions)
    {
      ...
      {
        if (CodeInstructionExtensions.Calls(list[index], AccessTools.Method(typeof (GameObject), "GetComponent", new Type[0], new Type[1]
        {
          typeof (ZNetView)
        })))
        {
          list[index] = new CodeInstruction(OpCodes.Call, (object) AccessTools.Method(typeof (Plantable_Patch), "PlantGrowth", (Type[]) null, (Type[]) null));
    ...
    private static ZNetView PlantGrowth(GameObject newObject, Plant oldPlant)
    {
      ...
      MoveableBaseRootComponent componentInParent = ((Component) oldPlant).GetComponentInParent<MoveableBaseRootComponent>();
