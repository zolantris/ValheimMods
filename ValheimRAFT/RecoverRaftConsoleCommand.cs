...
internal class RecoverRaftConsoleCommand : ConsoleCommand
{
	...
	public override void Run(string[] args)
	{
		...
		foreach (Collider collider in array)
		{
			...
			if (!(netview != null) || netview.m_zdo == null || (bool)netview.GetComponentInParent<MoveableBaseRootComponent>())
			{
				continue;
			}
			...
			{
				Vector3 partOffset = netview.m_zdo.GetVec3(MoveableBaseRootComponent.MBPositionHash, Vector3.zero);
			}
		}
		...
		if (args.Length != 0 && args[0] == "confirm")
		{
			foreach (ZDOID zdoid in unattached.Keys)
			{
				...
				GameObject ship = Object.Instantiate(shipPrefab, list[0].transform.position, list[0].transform.rotation);
				...
				foreach (ZNetView piece in list)
				{
					...
					piece.transform.localPosition = piece.m_zdo.GetVec3(MoveableBaseRootComponent.MBPositionHash, Vector3.zero);
					piece.transform.localRotation = piece.m_zdo.GetQuaternion(MoveableBaseRootComponent.MBRotationHash, Quaternion.identity);
