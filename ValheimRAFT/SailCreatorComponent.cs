...
public class SailCreatorComponent : MonoBehaviour
{
	...
	public void Awake()
	{
		...
		if (m_sailCreators.Count > 0 && (m_sailCreators[0].transform.position - base.transform.position).sqrMagnitude > SailComponent.m_maxDistanceSqr)
		{
		...
		if (m_sailCreators.Count >= m_sailSize)
		{
			...
			Vector3 center = (m_sailCreators[0].transform.position + m_sailCreators[1].transform.position) / 2f;
			...
			MoveableBaseRootComponent mbroot = m_sailCreators[0].GetComponentInParent<MoveableBaseRootComponent>();
			if ((bool)mbroot)
			{
