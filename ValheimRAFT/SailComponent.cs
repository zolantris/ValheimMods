...
public class SailComponent : MonoBehaviour, Interactable, Hoverable
{
	...
	public void SetAllowSailShrinking(bool allow)
	{
		...
		{
			m_sailFlags = (allow ? (m_sailFlags | SailFlags.AllowSailShrinking) : (m_sailFlags & ~SailFlags.AllowSailShrinking));
	...
	public void SetDisableCloth(bool allow)
	{
		if ((bool)m_mastComponent)
		{
			m_sailFlags = (allow ? (m_sailFlags | SailFlags.DisableCloth) : (m_sailFlags & ~SailFlags.DisableCloth));
	...
	public void LoadFromMaterial()
	{
		...
		CustomTextureGroup.CustomTexture mainGroup = CustomTextureGroup.Get("Sails").GetTextureByHash(mainTex.name.GetStableHashCode());
		if (mainGroup != null)
		{
		...
		CustomTextureGroup.CustomTexture patternGroup = CustomTextureGroup.Get("Patterns").GetTextureByHash(patternTex.name.GetStableHashCode());
		...
		CustomTextureGroup.CustomTexture logoGroup = CustomTextureGroup.Get("Logos").GetTextureByHash(logoTex.name.GetStableHashCode());
		if (logoGroup != null)
		{
	...
	{
		ZLog.Log($"CreateSailMesh(): {m_sailCorners.Count} m_lockedSailCorners: {m_lockedSailCorners} ({(int)m_lockedSailCorners}) m_lockedSailSides: {m_lockedSailSides} ({(int)m_lockedSailSides})");
		...
		if (m_sailCorners.Count == 3)
		{
			...
			while (true)
			{
				float dist = (mesh.vertices[mesh.triangles[0]] - mesh.vertices[mesh.triangles[1]]).sqrMagnitude;
				if (dist < sqrSubDist)
				{
					break;
				}
	...
	public void UpdateCoefficients()
	{
		...
		ClothSkinningCoefficient[] coefficients = ((mesh.vertexCount == m_sailCloth.coefficients.Length) ? m_sailCloth.coefficients : new ClothSkinningCoefficient[mesh.vertexCount]);
		...
		if (m_sailCorners.Count == 3)
		{
			...
			for (int k = 0; k < mesh.vertices.Length; k++)
			{
				...
				if (m_lockedSailSides.HasFlag(SailLockedSide.A) && Mathf.Abs(Vector3.Dot((m_sailCorners[0] - mesh.vertices[k]).normalized, sideA2)) >= 0.9999f)
				{
					coefficients[k].maxDistance = 0f;
				}
				if (m_lockedSailSides.HasFlag(SailLockedSide.B) && Mathf.Abs(Vector3.Dot((m_sailCorners[1] - mesh.vertices[k]).normalized, sideB2)) >= 0.9999f)
				{
					coefficients[k].maxDistance = 0f;
				}
				if (m_lockedSailSides.HasFlag(SailLockedSide.C) && Mathf.Abs(Vector3.Dot((m_sailCorners[2] - mesh.vertices[k]).normalized, sideC2)) >= 0.9999f)
				{
					coefficients[k].maxDistance = 0f;
				}
			}
		...
		else if (m_sailCorners.Count == 4)
		{
			...
			for (int j = 0; j < mesh.vertices.Length; j++)
			{
				...
				if (m_lockedSailSides.HasFlag(SailLockedSide.A) && Mathf.Abs(Vector3.Dot((m_sailCorners[0] - mesh.vertices[j]).normalized, sideA)) >= 0.9999f)
				{
					coefficients[j].maxDistance = 0f;
				}
				if (m_lockedSailSides.HasFlag(SailLockedSide.B) && Mathf.Abs(Vector3.Dot((m_sailCorners[1] - mesh.vertices[j]).normalized, sideB)) >= 0.9999f)
				{
					coefficients[j].maxDistance = 0f;
				}
				if (m_lockedSailSides.HasFlag(SailLockedSide.C) && Mathf.Abs(Vector3.Dot((m_sailCorners[2] - mesh.vertices[j]).normalized, sideC)) >= 0.9999f)
				{
					coefficients[j].maxDistance = 0f;
				}
				if (m_lockedSailSides.HasFlag(SailLockedSide.D) && Mathf.Abs(Vector3.Dot((m_sailCorners[3] - mesh.vertices[j]).normalized, sideD)) >= 0.9999f)
				{
					coefficients[j].maxDistance = 0f;
				}
			}
	...
	public void SetPattern(int hash)
	{
		...
		m_patternHash = hash;
		CustomTextureGroup.CustomTexture customtexture = CustomTextureGroup.Get("Patterns").GetTextureByHash(hash);
	...
	public void SetMain(int hash)
	{
		...
		m_mainHash = hash;
		CustomTextureGroup.CustomTexture customtexture = CustomTextureGroup.Get("Sails").GetTextureByHash(hash);
		if (customtexture != null && (bool)customtexture.Texture && (bool)m_mesh)
		{
	...
	public void SetLogo(int hash)
	{
		...
		m_logoHash = hash;
		CustomTextureGroup.CustomTexture customtexture = CustomTextureGroup.Get("Logos").GetTextureByHash(hash);
		if (customtexture != null && (bool)customtexture.Texture && (bool)m_mesh)
		{
