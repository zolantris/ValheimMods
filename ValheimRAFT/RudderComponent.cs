	...
	public void UpdateIK(Animator animator)
	{
		...
		if (!m_targetLeftHand && !m_targetRightHand)
		{
			...
			if (left.z < 0.2f)
			{
				...
				m_targetLeftHand = GetNearestSpoke(base.transform.TransformPoint(m_leftHandPosition + offsetY2));
				m_movingLeftAlpha = Time.time;
			}
			else if (right.z > -0.2f)
			{
				Vector3 offsetY = new Vector3(0f, (right.y > 0.5f) ? (-2f) : 2f, 0f);
				m_targetRightHand = GetNearestSpoke(base.transform.TransformPoint(m_rightHandPosition + offsetY));
		...
		float leftHandIKWeight = Mathf.Sin(leftHandAlpha * (float)Math.PI) * (1f - m_holdWheelTime) + m_holdWheelTime;
		float rightHandIKWeight = Mathf.Sin(rightHandAlpha * (float)Math.PI) * (1f - m_holdWheelTime) + m_holdWheelTime;
		...
		Vector3 leftHandPos = (m_targetLeftHand ? Vector3.Lerp(m_currentLeftHand.transform.position, m_targetLeftHand.transform.position, leftHandAlpha) : m_currentLeftHand.transform.position);
		Vector3 rightHandPos = (m_targetRightHand ? Vector3.Lerp(m_currentRightHand.transform.position, m_targetRightHand.transform.position, rightHandAlpha) : m_currentRightHand.transform.position);
		Vector3 rightHandRot = (m_targetLeftHand ? Vector3.Slerp(m_currentLeftHand.transform.rotation.eulerAngles, m_targetLeftHand.transform.rotation.eulerAngles, leftHandAlpha) : m_currentLeftHand.transform.rotation.eulerAngles);
