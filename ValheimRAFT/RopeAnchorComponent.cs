...
public class RopeAnchorComponent : MonoBehaviour, Interactable, Hoverable
{
	...
	private static Dictionary<string, string[]> GetAttachmentPoints()
	{
		...
		attachmentPoints.Add("Karve(Clone)", new string[5] { "ship/ropes/mastrope", "ship/ropes/RopeAttachLeft_bottom_front", "ship/ropes/RopeAttachLeft_bottom_back", "ship/ropes/RopeAttachRight_bottom_front (1)", "ship/ropes/RopeAttachRight_bottom_back (1)" });
		attachmentPoints.Add("VikingShip(Clone)", new string[8] { "interactive/front", "interactive/mast", "interactive/ladder_right", "interactive/ladder_left", "ship/visual/ropes/RopeAttachLeft_bottom_front", "ship/visual/ropes/RopeAttachLeft_bottom_back", "ship/visual/ropes/RopeAttachRight_bottom_front (1)", "ship/visual/ropes/RopeAttachRight_bottom_back (1)" });
		attachmentPoints.Add("Deer(Clone)", new string[1] { "Visual/CG/Pelvis/Spine/Spine1/Spine2/Neck" });
		attachmentPoints.Add("Boar(Clone)", new string[1] { "Visual/CG/Pelvis/Spine/Spine1/Spine2/Neck" });
		attachmentPoints.Add("Boar_piggy(Clone)", new string[1] { "Visual/CG/Pelvis/Spine/Spine1/Spine2/Neck" });
	...
	{
		if (m_attachmentPoints.TryGetValue(go.name, out var points) && target.Index >= 0 && points.Length > target.Index)
		{
	...
	private void AttachRope(GameObject gameObject, byte index)
	{
		...
		if ((bool)nv && nv.m_zdo != null)
		{
			...
			RopeAttachmentTarget id = new RopeAttachmentTarget(ZDOPersistantID.Instance.GetOrCreatePersistantID(nv.m_zdo), index);
	...
	{
		int parentid = ZDOPersistantID.Instance.GetOrCreatePersistantID(ropeAnchorComponent.GetParentZDO());
		if (!RemoveRopeWithID(new RopeAttachmentTarget(parentid, 0)) && !(ropeAnchorComponent == this) && !ropeAnchorComponent.RemoveRopeWithID(new RopeAttachmentTarget(parentid, 0)))
		{
	...
	public void LateUpdate()
	{
		...
		if (m_rope.enabled && (bool)Player.m_localPlayer)
		{
			...
			{
				m_rope.SetPosition(1, ((Humanoid)Player.m_localPlayer).m_visEquipment.m_rightHand.transform.position);
			}
		}
		...
		for (int i = 0; i < m_updatingRopes.Count; i++)
		{
			...
			if ((bool)rope.m_ropeObject && (bool)rope.m_ropeTarget)
			{
				...
				if (m_nview.IsOwner() && (ropeDistance > m_maxRopeDistance || ropeDistance > rope.m_ropeAttachDistance + 8f))
				{
	...
	private void CheckRopes()
	{
		for (int i = 0; i < m_ropes.Count; i++)
		{
			...
			Transform targetTransform = GetAttachmentTransform(rope.m_ropeTarget, rope.m_ropeAnchorTarget);
			...
			if ((!rb && rope.m_ropeTarget.transform.parent == base.transform.parent) || rb == m_rigidbody)
			{
				...
				rope.m_rope.SetPosition(0, rope.m_rope.transform.InverseTransformPoint(base.transform.position));
				rope.m_rope.SetPosition(1, rope.m_rope.transform.InverseTransformPoint(rope.m_targetTransform.position));
			}
			else
			{
	...
	private void GetRopesFromZDO(ICollection<RopeAttachmentTarget> ropeIds)
	{
		try
		{
			...
			if (bytesPkg != null && bytesPkg.Length != 0)
			{
				...
				{
					ropeIds.Add(new RopeAttachmentTarget((version <= 1) ? GetRopeTarget(pkg.ReadZDOID()) : pkg.ReadInt(), pkg.ReadByte()));
				}
			}
		}
		catch (Exception)
